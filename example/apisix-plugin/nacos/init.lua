...
local service_list_path = 'ns/service/list?pageNo=1&pageSize=99999'
local instance_list_path = 'ns/instance/list?healthyOnly=false&serviceName='
...
local function discovery_nacos_callback(data, event, source, pid)
    applications = data
    log.warn("update local variable application, event is: ", event,
...
local function request(request_uri, path, body, method, basic_auth)
    ...
    log.warn("request-request_uri: ", url)
    log.warn("request-query_body: ", body)
    log.warn("request-res.body: ", json_str)
...
local function get_all_nacos_service(base_uri, token_param)

    local services = {}
    local data, err
    local namespace_id = default_namespace_id
    ...
    local query_path = service_list_path .. token_param .. namespace_param .. group_name_param
    ...
    log.warn("get_all_nacos_service-data: ", core.json.encode(data, true))
    log.warn("get_all_nacos_service-data.doms: ", core.json.encode(data.doms, true))

    local values = data.doms

    log.warn("get_all_nacos_service-data.doms--values2222: ", core.json.encode(values, true))
    ...
    for _, value in ipairs(values) do
        ...
        local dup = de_duplication(services, namespace_id, group_name, value, '')
        if dup then
            goto CONTINUE
        end
        core.table.insert(services, {
...
local function get_nacos_services()
    ...
    log.warn("get_nacos_services: ", core.json.encode(services, true))
    return services
end
...
local function fetch_full_registry(premature)
    ...
    local infos = get_all_nacos_service(base_uri, token_param)
    ...
    log.warn("fetch_full_registry-infos: ", core.json.encode(infos, true))
    ...
    for _, service_info in ipairs(infos) do
        ...
        for _, host in ipairs(data.hosts) do
            ...
            log.warn("fetch_full_registry-nodes: ", core.json.encode(nodes, true))
        end

        log.warn("fetch_full_registry-up_app222: ", core.json.encode(up_apps, true))
    ...
    log.warn("fetch_full_registry-up_apps: ", core.json.encode(up_apps, true))
    log.warn("fetch_full_registry-applications: ", core.json.encode(applications, true))
    log.warn("fetch_full_registry-up_apps-md5: ", new_apps_md5sum)
    log.warn("fetch_full_registry-applications-md5: ", old_apps_md5sum)
...
function _M.nodes(service_name, discovery_args)
    ...
    log.warn("_M.nodes-service_name: ", service_name)
    log.warn("_M.nodes-namespace_id: ", namespace_id)
    log.warn("_M.nodes-group_name: ", group_name)
    log.warn("_M.nodes-discovery_args: ", core.json.encode(discovery_args, true))
    ...
    if not applications[namespace_id] or not applications[namespace_id][group_name] then
        log.warn("_M.nodes-111111: null")
        return nil
    end
    log.warn("_M.nodes-discovery_args-222222: ", core.json.encode(applications[namespace_id][group_name][service_name], true))
    log.warn("_M.nodes-discovery_args-333333: ", core.json.encode(applications, true))
...
function _M.init_worker()
    ...
    log.warn('_M.init_worker-default_weight:', default_weight)
    local fetch_interval = local_conf.discovery.nacos.fetch_interval
    log.warn('_M.init_worker-fetch_interval:', fetch_interval)
